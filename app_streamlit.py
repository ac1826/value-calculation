...
def _build_excel_col_format_map(columns):
    ...
    for c in columns:
        ...
        if name in EXCEL_INT_COLS:
            out[name] = "#,##0"
        elif name in EXCEL_TWO_DEC_COLS:
            out[name] = "0.00"
...
def build_overview_table_for_day(overview, sel, df_lw, total_qty_df=None, pr_raw=None, percent_as_text=True, round_values=True):
...
if overview is None or overview.empty:
    ...
    try:
        ...
        for col in ["产值(元/kg)", "生肉产量(吨)", "销量(吨)", "宰鸡量(千只)", "均重(kg/只)", "产成率(%)", "产销率(%)"]:
    ...
    try:
        ...
        total_qty_df = None
    disp = build_overview_table_for_day(overview, sel, df_lw, total_qty_df=total_qty_df, percent_as_text=True, round_values=True)
    export_daily_src = build_overview_table_for_day(overview, sel, df_lw, total_qty_df=total_qty_df, percent_as_text=False, round_values=False)
            ...
            if pd.notna(sel_dt):
                ...
                if days_in_range:
                    ...
                    for _d in days_in_range:
                        day_df = build_overview_table_for_day(overview, _d, df_lw, percent_as_text=False, round_values=False)
