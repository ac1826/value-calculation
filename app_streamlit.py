...
def _compute_restored_main_side_qty_amt_for_period(overview, minors, df_lw, restore_mapping, start, end):
    ...
    days = sorted(pd.to_datetime(overview.loc[mask, "\u65e5\u671f"], errors="coerce").dropna().dt.normalize().unique())
    if not days:
        return np.nan, np.nan, np.nan, np.nan

    main_fixed_parts = {"\u817f\u7c7b", "\u91cc\u808c\u7c7b", "\u7fc5\u7c7b", "\u722a\u7c7b"}
    chest_parts = {"\u80f8\u7c7b-\u80f8", "\u80f8\u7c7b-\u80f8\u76ae"}
    side_parts = set(BASE_FOR_TOTAL) - main_fixed_parts - chest_parts
    ...
    for day_value in days:
        ...
        proj_day = ov_day.groupby("\u9879\u76ee", as_index=False)[["\u4ea7\u91cf(kg)", "\u542b\u7a0e\u91d1\u989d"]].sum(min_count=1)
        qty_map = {}
        unit_map = {}
        for _, row in proj_day.iterrows():
        ...
        delta_map = {
            proj: float(inc_map.get(proj, 0.0)) - float(removed_map.get(proj, 0.0))
            for proj in (set(qty_map) | set(inc_map) | set(removed_map))
        }

        for proj in sorted(main_fixed_parts):
            qty = qty_map.get(proj, 0.0) + delta_map.get(proj, 0.0)
            unit = unit_map.get(proj, np.nan)
            main_qty_total += qty
            if pd.notna(unit):
                main_amt_total += qty * unit

        chest_component_present = any(
            abs(qty_map.get(proj, 0.0)) > 1e-9 or abs(delta_map.get(proj, 0.0)) > 1e-9
            for proj in chest_parts
        )
        if chest_component_present:
            chest_qty = 0.0
            chest_amt = 0.0
            for proj in chest_parts:
                qty = qty_map.get(proj, 0.0) + delta_map.get(proj, 0.0)
                ...
                if pd.notna(unit):
                    chest_amt += qty * unit
            chest_direct_qty = delta_map.get("\u80f8\u7c7b", 0.0)
            chest_direct_unit = unit_map.get("\u80f8\u7c7b", np.nan)
            chest_qty += chest_direct_qty
            if pd.notna(chest_direct_unit):
                chest_amt += chest_direct_qty * chest_direct_unit
        else:
            chest_qty = qty_map.get("\u80f8\u7c7b", 0.0) + delta_map.get("\u80f8\u7c7b", 0.0)
            chest_unit = unit_map.get("\u80f8\u7c7b", np.nan)
            chest_amt = chest_qty * chest_unit if pd.notna(chest_unit) else 0.0
        main_qty_total += chest_qty
        main_amt_total += chest_amt

        for proj in sorted(side_parts):
            qty = qty_map.get(proj, 0.0) + delta_map.get(proj, 0.0)
            unit = unit_map.get(proj, np.nan)
            side_qty_total += qty
            if pd.notna(unit):
                side_amt_total += qty * unit

    if not have_day:
        return np.nan, np.nan, np.nan, np.nan
    return main_qty_total, main_amt_total, side_qty_total, side_amt_total
